                ...
                for index, row in enumerate(table.find_all('tr')):
                    ...
                    if len(columns) > 0:
                        ...
                        if functionality_name in response:
                            ...
                            existing_total = columns[1].get_text(strip=True)
                            ...
                            total_value = sum([counts['dev_total_cnt'], counts['uat_total_cnt']])
                            ...
                            if (columns[2].string != existing_dev_mnl or
                                ...
                                columns[5].string != existing_uat_aut or 
                                columns[1].string != existing_total):
...
def count_rows_in_first_table(table):
    ...
    try :
        for index, row in enumerate(rows[keyword_row_start_index:]):
            cells_to_use = row.find_all(['th', 'td'])

            # Get the text content of the chosen cells
            row_text = ' '.join(cell.text.strip() for cell in cells_to_use)
            ...
            if 'Negative / Exception Testing' in row_text or 'Other Non-Functional Testing' in row_text \
                or 'Feature / Module/ Pipeline Testing' in row_text or 'Black Box/Validation/Pipeline Testing' in row_text \
                or 'Unit Testing' == row_text.strip() :
                print(f"Skipping row {index + keyword_row_start_index} due to excluded content")
                continue
            test_technique_data = cells_to_use[2].get_text().lower()
            if test_technique_data.strip() == 'manual':
                manual_count +=1
                print("Its a manual Row -- Current Manual Row is ", manual_count)
            if test_technique_data.strip() == 'automated':
                automated_count +=1
                print("Its a automated Row -- Current Manual Row is ", automated_count)

            total_count += 1
            print("total count is", total_count)  
    except Exception as e: 
        print("Exception occurred for row",row_text)
